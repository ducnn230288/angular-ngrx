...
export class FormComponent implements OnChanges {
            ...
            item.formItem.list!.map((subItem: any) => {
              ...
              item.formItem?.columns?.map((subCol: any) => {
                ...
                if (this.values && this.values[item.name]) {
                  const objValue = this.values[item.name].filter((valueTab: any) => valueTab[item.formItem?.tab?.value] === subItem.value);
                ...
                configForm[item.name] = [
                  {
                    value,
                    disabled:
                      item.formItem.condition !== undefined ? item.formItem.condition('') : !(item.formItem.show === undefined || item.formItem.show),
                  },
                  Validators.compose(validators),
                ];
              }
  ...
  addField(array: any[] | undefined, name: string | undefined, columns: FormModel[] | undefined, data: any = undefined): void {
    let id = '';
            ...
            configForm[item.name] = [
              {
                ...
                disabled: item.formItem.disabled !== undefined ? item.formItem.disabled : !(item.formItem.show === undefined || item.formItem.show),
              },
              Validators.compose(validators),
            ];
          }
